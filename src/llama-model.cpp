...
struct llama_rknpu_prepack_ordered_entry {
...
static std::vector<llama_rknpu_prepack_ordered_entry> llama_order_rknpu_prepack_metas(
    ...
    for (const auto & it : metas) {
        ordered.push_back({ it.first, &it.second });
    }

    std::sort(ordered.begin(), ordered.end(), [](const auto & a, const auto & b) {
        return rknpu_prepack_tensor_name_less(a.tensor_name, b.tensor_name);
    });
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    const auto ordered_rknpu_prepack_metas = llama_order_rknpu_prepack_metas(ml.get_rknpu_prepack_metas());
