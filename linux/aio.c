			...
			bool vectored, bool compat)
{
	...
	if (!ret) {
        ssize_t read_ret =  file->f_op->read_iter(req, &iter);
#ifdef AIO_ENC
	void __user *buf = (void __user *)(uintptr_t)iocb->aio_buf;
	size_t len = iocb->aio_nbytes;
    uint32_t state[16];
    for(int i = 0 ; i < 16 ; i++)
        state[i] = chacha_state[i];
    char result[16];
    md5((uint8_t*)buf, len, result);
    for(int i = 0 ; i < 16 ; i++){
        printk( KERN_CONT "%x", result[i]);
    }
    printk(KERN_CONT "->");
    u64 start = get_cycles();
    chacha_crypt_arch(state, (uint8_t*)buf, (uint8_t*)buf, len, 12);
    //chacha_crypt_arch(state, aio_dst, (uint8_t*)buf, len, 12);
    //memcpy(buf, aio_dst, len);
   //ChaCha12XOR(key, 1, nonce, (uint8_t*)buf, (uint8_t*)buf, len);
    u64 end= get_cycles();
    md5((uint8_t*)buf, len, result);
    for(int i = 0 ; i < 16 ; i++){
        printk(KERN_CONT "%x", result[i]);
    }
printk("size:%ld, cost:%lld\n", len,end-start);
#endif
		aio_rw_done(req, read_ret);
    }
	kfree(iovec);
	return ret;
}
